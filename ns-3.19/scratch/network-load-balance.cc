...
// 交换机监测间隔时间，单位为纳秒。这表示每隔10微秒，会对交换机进行监测
uint32_t switch_mon_interval = 10000;  // ns
// CNP（Credit Notification Packet）监测的开始时间
uint64_t cnp_mon_start;                // ns
// CNP监测的时间分辨率，
uint64_t cnp_monitor_bucket = 100000;  // ns
// IRN（Incipient Rate Notification）监测的开始时间
...
struct Interface {
    ...
    Interface() : idx(0), up(false) {}  // idx 被初始化为0，up 被初始化为假（false）
};
map<Ptr<Node>, map<Ptr<Node>, Interface>> nbr2if;  // 网络节点之间的接口信息，以及每个接口的参数
// Mapping destination to next hop for each node: <node, <dest, <nexthop0, ...> > >
map<Ptr<Node>, map<Ptr<Node>, vector<Ptr<Node>>>> nextHop;  // 每个节点到目标节点之间的下一跳节点的信息
// 用于存储不同节点之间的各种参数，如延迟、传输延迟、带宽、带宽延迟乘积（BDP）、往返时间（RTT）
...
/*
key为TOR交换机的标识号或ID，value为与特定TOR交换机相关联的上行接口的标识号
用于跟踪每个TOR交换机与哪些上行接口连接，以便监测和分析上行流量
*/
std::map<uint32_t, std::vector<uint32_t>> torId2UplinkIf;
/*
key为TOR交换机的标识号或ID，value为与特定TOR交换机相关联的下行接口的标识号
用于跟踪每个TOR交换机与哪些下行接口连接，以便监测和分析下行流量
*/
...
uint64_t get_nic_rate(NodeContainer &n) {
    ...
    // 通过遍历节点容器中的每个服务器节点，累积其NIC速率，最后计算平均值并返回
}
...
int main(int argc, char *argv[]) {
    ...
    for (uint32_t i = 0; i < node_num; i++) {
        if (n.Get(i)->GetNodeType() == 0) {  // is server
            ...
            serverAddress[i] = Settings::node_id_to_ip(i);  // 为每个服务器分配ip地址
        }
    }
    ...
    //
    Ptr<RateErrorModel> rem = CreateObject<RateErrorModel>();  // 表示通道的错误模型
    Ptr<UniformRandomVariable> uv = CreateObject<UniformRandomVariable>();  // 用于生成均匀分布的随机数
    // 将先前创建的均匀随机数生成器uv设置为通道的错误模型rem中的随机数生成器，意味着随机性将通过uv来处理通道的错误率
    rem->SetRandomVariable(uv);
    // 设置均匀分布随机数生成器的流（stream）编号为50，不同的流编号可以用于生成不同的随机序列。
    uv->SetStream(50);
    // 配置错误模型的属性，将错误率设置为error_rate_per_link的值，表示通道上的错误率将由这个值确定
    rem->SetAttribute("ErrorRate", DoubleValue(error_rate_per_link));
    // 配置错误模型的属性，指定错误单位为数据包（"ERROR_UNIT_PACKET"），表示错误率的度量将以每个数据包的错误数为单位
    ...
    for (uint32_t i = 0; i < link_num; i++) {
        ...
        // 这应该在下面的自动分配（ipv4.Assign(d)）之前，
        // 因为我们希望我们的IP成为主IP（IP地址列表中的第一个），以便全局路由基于我们的IP
        NetDeviceContainer d = qbb.Install(snode, dnode);
        if (snode->GetNodeType() == 0) {
            Ptr<Ipv4> ipv4 = snode->GetObject<Ipv4>();
            ipv4->AddInterface(d.Get(0));
            // serverAddress[src] 是服务器的 IP 地址，Ipv4Mask(0xff000000) 是子网掩码
            // 这将创建一个 IP 地址，其中的前8位与 0xff 匹配，以确保它们属于同一个子网
        ...
        char ipstring[16];
        Ipv4Address x;
        sprintf(ipstring, "10.%d.%d.0", i / 254 + 1, i % 254 + 1);  // 确保每个节点都有唯一的IP地址
        ipv4.SetBase(ipstring, "255.255.255.0");
        ipv4.Assign(d);  // 将IP地址分配给网络设备容器 d 中的所有设备，以确保节点之间的连接
    ...
    // 用于在仿真中建立节点ID和IP地址之间的双向映射关系
    // 这有助于在仿真中快速查找节点的ID或IP地址，并确保服务器节点分配了有效的IP地址
    ...
    for (uint32_t i = 0; i < node_num; i++) {
        if (n.Get(i)->GetNodeType() == 1) {  // is switch
            ...
            for (uint32_t j = 1; j < sw->GetNDevices(); j++) {
                ...
                // 计算 PFC 头部空间（headroom）
                // 根据链路的带宽、RTT 和其他相关因素计算出足够的 PFC 头部空间，
                // 以确保交换机在处理 PFC 帧时有足够的容量来包含所需的控制信息
                uint32_t headroom = rate * delay / 8 / 1000000000 * 2 + 2 * sw->m_mmu->MTU;
                sw->m_mmu->ConfigHdrm(j, headroom);
            }
            sw->m_mmu->ConfigNPort(sw->GetNDevices() - 1);  // 配置端口数量
    ...
    /*
    这段代码注释提供了一些关于安装 RDMA 驱动（Mellanox 驱动）时的参数配置信息。这些参数控制了 RDMA 驱动的行为，特别是在网络通信过程中调整数据传输速率的行为。以下是其中一些参数的简要说明：
    1. `clamp_tgt_rate (false)`：当接收到 CNP（Congestion Notification Packet）时，是否始终将目标传输速率更新为当前速率。
    2. `clamp_tgt_rate_after_time_inc (true)`：当接收到 CNP 时，是否始终将目标传输速率更新为当前速率，即使最后一次速率增加事件是由计时器触发的，而不仅仅是由字节计数器触发的。
    3. `initial_alpha_value (1023)`：初始化 Alpha 值，这通常用于控制拥塞控制算法中的速率控制行为。
    4. `rate_reduce_monitor_period (4)`：用于确定降低流速的最小时间间隔。如果在此时间间隔内接收到 CNP，则在下一个 `rate_reduce_monitor_period` 时间间隔开始时将流速降低为 `(1-Alpha/Gd) * CurrentRate`。`rpg_gd` 以 `log2(Gd)` 的形式给出，其中 `Gd` 必须为 2 的幂。
    5. `rpg_gd (11)`：如果接收到 CNP，则在下一个 `rate_reduce_monitor_period` 时间间隔开始时将流速降低为 `(1-Alpha/Gd) * CurrentRate`。在该模拟器中，通常将 `(alpha / gd)` 设置为约 0.5，所以不需要设置 `rpg_gd` 参数。
    6. `rate_to_set_on_first_cnp (0)`：首次接收到 CNP 时为流量设置的速率，单位为 Mbps。
    7. `rpg_time_reset (300us)`：速率增加事件的时间计数器。
    8. `rpg_threshold (1)`：在快速恢复（Fast Recovery）、主动增加（Active Increase）和超快速增加（Hyper Active Increase）模式之间切换所需的速率增加事件数量。
    9. `dce_tcp_rtt (1)`：用于计算 Alpha 移动平均值的采样窗口。
    10. `dce_tcp_g (1019)`：用于计算 Alpha 移动平均值的采样中的权重。
    11. `rpg_byte_reset (32767)`：速率增加事件的字节计数器。
    12. `rpg_min_dec_fac (50)`：速率可以降低的最大因子，例如，2 表示最大速率可以减小为当前速率的一半。
    这些参数可能用于调整 RDMA 驱动的拥塞控制算法，以适应特定网络条件和应用需求。不同的参数设置可以影响数据传输速率的调整方式，以应对网络拥塞和其他因素。
    */
    /**
     ...
     */
     // 计算网络中每对服务器节点之间的 BDP（带宽时延积）和延迟（RTT）并找到最大的 BDP 值（maxBdp）
    maxRtt = maxBdp = 0;
    ...
    for (uint32_t i = 0; i < node_num; i++) {
        ...
        for (uint32_t j = i + 1; j < node_num; j++) {
            ...
            uint64_t delay = pairDelay[n.Get(i)][n.Get(j)];  // 结点延迟
            uint64_t txDelay = pairTxDelay[n.Get(i)][n.Get(j)];  // 传输延迟
            // 根据节点对之间的延迟（pairDelay）和传输延迟（pairTxDelay）计算 RTT。RTT 是延迟的两倍加上传输延迟。
            uint64_t rtt = delay * 2 + txDelay;
            uint64_t bw = pairBw[n.Get(i)][n.Get(j)];  // 带宽
            // 计算 BDP，RTT 与带宽（pairBw）的乘积除以 1000000000（1 Gbps）再除以 8，将 BDP 转换为字节数
    ...
    for (auto &pair : link_pairs) {
        ...
        // host-switch link
        // 如果链路的一端是服务器，而另一端是顶部交换机，那么将这个顶部交换机标记为 ToR（Top-of-Rack）交换机
