...
def main():
    ...
    parser.add_argument('--algorithm', dest='algorithm', action='store', 
                        default='all-reduce', help='collective communication algorithm')
    parser.add_argument('--implementation', dest='implementation', action='store', 
                        help='the implementation of the algorithm')
    parser.add_argument('--physical_dims', dest='physical_dims',
                        help='the physical dimensions')
    ...
    # 重新编写生成结果的标识
    config_ID = ''
    if enabled_pfc == 1 and enabled_irn == 0:
        config_ID += 'pfc_'
    else:
        config_ID += 'irn_'
    ...
    # flow = "L_{load:.2f}_CDF_{cdf}_N_{n_host}_T_{time}ms_B_{bw}_A_{algo}_I_{impl}_P_{phy_dims}_flow".format(
    #     load=hostload, 
    #     cdf=args.cdf, 
    #     n_host=n_host, 
    ...
    #     phy_dims=args.physical_dims)
    flow = "L_{load:.2f}_N_{n_host}_T_{time}ms_B_{bw}_A_{algo}_I_{impl}_P_{phy_dims}_flow".format(
        load=hostload, 
        n_host=n_host, 
        time=int(float(args.simul_time)*1000), 
        bw=bw,
    ...
    if (exists(os.getcwd() + "/config/" + flow + ".txt")):
        ...
        print("Input traffic file with load:{load:.2f}, n_host:{n_host} already exists".format(
            load=hostload, n_host=n_host))
    else:  # make the input traffic file
        print("Generate a input traffic file...")
        # 重新编写流量生成的命令
        print("python3 ./traffic_gen/flow_gen.py -n {n_host} -l {load} -b {bw} -t {time} -a {algo} -i{impl} -p {phy_dims} -o {output}".format(
            n_host=n_host,
            load=hostload / 100.0,
            bw=args.bw + "G",
        ...
        os.system("python3 ./traffic_gen/flow_gen.py -n {n_host} -l {load} -b {bw} -t {time} -a {algo} -i{impl} -p {phy_dims} -o {output}".format(
            n_host=n_host,
            load=hostload / 100.0,
            bw=args.bw + "G",
            time=args.simul_time,
    ...
    with open("./mix/.history", "a") as history:
        history.write("{simulday},{config_ID},{cc_mode},{lb_mode},{cwh_tx_expiry_time},{cwh_extra_reply_deadline},{cwh_path_pause_time},{cwh_extra_voq_flush_time},{cwh_default_voq_waiting_time},{pfc},{irn},{has_win},{var_win},{topo},{bw},{cdf},{load},{time},{algo},{impl},{phy_dims}\n".format(
            simulday=simulday,
            config_ID=config_ID,
            cc_mode=cc_mode,
            lb_mode=lb_mode,
